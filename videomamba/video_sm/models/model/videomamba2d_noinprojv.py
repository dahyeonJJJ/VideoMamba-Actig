...
from mamba_ssm.modules.mamba_simple import Mamba, Mamba2d, Mamba2d_noinprojv
...
def create_block(
    ...
    if ssm_cfg is None:
        ssm_cfg = {}
    mixer_cls = partial(Mamba2d_noinprojv, layer_idx=layer_idx, bimamba=bimamba, vertical=vertical, token_seq=token_seq, **ssm_cfg, **factory_kwargs)
...
@register_model
def videomamba2d_noinprojv_tiny(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, 
        embed_dim=192, 
        depth=24, 
...
@register_model
def videomamba2d_noinprojv_small(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, 
        embed_dim=384, 
        depth=24, 
...
@register_model
def videomamba2d_noinprojv_middle(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, 
        embed_dim=576, 
        depth=32, 
