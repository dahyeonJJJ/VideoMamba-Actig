...
from mamba_ssm.modules.mamba_simple import Mamba, Mamba_pruned, Mamba_pruned2, Mamba_pruned2_frame
...
def create_block(
    ...
    if ssm_cfg is None:
        ssm_cfg = {}
    mixer_cls = partial(Mamba_pruned2_frame, layer_idx=layer_idx, bimamba=bimamba, **ssm_cfg, **factory_kwargs)
...
@register_model
def videomamba_pruned_tiny(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, 
        embed_dim=192, 
        depth=24, 
...
@register_model
def videomamba_pruned_small(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, 
        embed_dim=384, 
        depth=24, 
...
@register_model
def videomamba_pruned_middle(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, 
        embed_dim=576, 
        depth=32, 
