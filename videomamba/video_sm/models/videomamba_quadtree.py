...
from mamba_ssm.modules.mamba_simple import Mamba_quadtree
...
def create_block(
    ...
    if ssm_cfg is None:
        ssm_cfg = {}
    mixer_cls = partial(Mamba_quadtree, layer_idx=layer_idx, bimamba=bimamba, token_seq=token_seq, stage_num=stage_num, depth_num=depth_num, block_depth=block_depth, **ssm_cfg, **factory_kwargs)
...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None):
        if False:
            ...
            tensor = tensor.detach().to(torch.float32).cpu().numpy().transpose(0, 2, 3, 4, 1).astype(np.float32)
            
            for t in range(T):
                raw_image = tensor[0, t]
                normalized_image = (raw_image - raw_image.min()) / (raw_image.max() - raw_image.min()) * 255
                image = Image.fromarray(normalized_image.astype(np.uint8), mode='RGB')
                filename = os.path.join(save_dir, f'img_t{t+1}.jpg')
                image.save(filename)
        ...
        hidden_states = hidden_states.mean(dim=1)  # (b, 1, d)
...
def videomamba_tiny_quadtree(pretrained=False, **kwargs):
    model = VisionMamba(
        ...
        embed_dim=192,
        depth=24, 
        block_depth=[12, 12], # [quadtree, bi-drection]
        rms_norm=True, 
        residual_in_fp32=True, 
        fused_add_norm=True, 
        **kwargs
