...
class SelectiveScanFn(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
        ...
        if delta_bias is not None:
            delta_bias = delta_bias.to(target_dtype)
...
class SelectiveScanFnOUT(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
        ...
        if not ctx.has_z:
            ctx.save_for_backward(u, delta, A, B, C, D, delta_bias, x)
            return (out, None, last_state) if return_last_state else (out, None)
        else:
            ctx.save_for_backward(u, delta, A, B, C, D, z, delta_bias, x, out)
            out_z = rest[0]
            # return (out, out_z, last_state) if return_last_state else (out, out_z)
            return (out_z, out, last_state) if return_last_state else (out_z, out)

    @staticmethod
    def backward(ctx, dout_z, dd=None, dlast_state=None):
        if not ctx.has_z:
            u, delta, A, B, C, D, delta_bias, x = ctx.saved_tensors
            z = None
            out = None
            ...
            u, delta, A, B, C, D, z, delta_bias, x, out = ctx.saved_tensors
        if dout_z.stride(-1) != 1:
            dout_z = dout_z.contiguous()

        du, ddelta, dA, dB, dC, dD, ddelta_bias, *rest = selective_scan_cuda.bwd(
            u, delta, A, B, C, D, z, delta_bias, dout_z, x, out, None,
...
def selective_scan_fn_out(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
                     return_last_state=False):
    """if return_last_state is True, returns (out, last_state)
    last_state has shape (batch, dim, dstate). Note that the gradient of the last state is
    not considered in the backward pass.
    """
    return SelectiveScanFnOUT.apply(u, delta, A, B, C, D, z, delta_bias, delta_softplus, return_last_state)
...
class MambaInnerFnNoOutProj_out(torch.autograd.Function):
...
def mamba_inner_fn_no_out_proj_out(
    xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
    A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True
):
    return MambaInnerFnNoOutProj_out.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus)
